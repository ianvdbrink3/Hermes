...
export function PersistentHermesChatV3() {
    ...
    async function poll() {
      try {
        ...
        if (!response.ok) {
          ...
          patchMessage({ status, updatedAt: new Date().toISOString(), lastPollError: undefined });
        }
      } catch {
        if (!stopped) {
          patchMessage({
          ...
          <div className={styles.thread} ref={threadRef}>
            {conversation.messages.map((message) => (
              <article key={message.id} className={message.role === "user" ? styles.you : message.role === "system" ? styles.system : styles.hermes}>
                ...
                {message.runId && message.status && TERMINAL.has(String(message.status).toLowerCase()) && (
